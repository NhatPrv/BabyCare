...
def enrich_with_who_reference(frame: pd.DataFrame) -> pd.DataFrame:
    ...
    who_df = pd.DataFrame(who_list, index=enriched.index)
    
    for column in who_df.columns:
        if column not in enriched.columns:
            enriched[column] = who_df[column]
        else:
            enriched[column] = enriched[column].where(enriched[column].notna(), who_df[column])
    return enriched
